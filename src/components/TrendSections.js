...
function Trendbar({timeseries, sections_top, chartType}) {
  const {t} = useTranslation();
  const refs = useRef([]);

  const dates = sections_top; 
  ...
  useEffect(() => {
    ...
    refs.current.forEach((ref, i) => {
      ...
      const statistic = SECTIONS_STATISTICS[i];
  ...
  return (
    <React.Fragment>
        {SECTIONS_STATISTICS.map((statistic, index) => {
