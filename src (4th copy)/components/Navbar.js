      ...
      {transitions.map(({item, key, props}) =>
        item ? (
          <animated.div key={key} style={props}>
            <Expand {...{pages, setExpand, darkMode1, windowSize}} />
          </animated.div>
        ) : (
...
function Expand({pages, setExpand, darkMode1, windowSize}) {
  ...
  return (
    <div className="expand" ref={expandElement} onMouseLeave={handleMouseLeave}>
      ...
      {windowSize.width < 768 && <SunMoon {...{darkMode1}} />}
...
const SunMoon = ({darkMode1}) => {
  return (
    <div className="SunMoon" onClick={darkMode1.toggle}>
      <div>
        {darkMode1.value ? <Icon.Sun color={'#ffc107'} /> : <Icon.Moon />}
      </div>
    </div>
  );
};
